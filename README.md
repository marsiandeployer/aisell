...
| Продукт | Описание | Конфиг | Webchat |
|---------|----------|--------|---------|
| **SimpleSite** | AI конструктор простых сайтов | `products/simple_site/product.yaml` | TBD |
| **SimpleDashboard** | AI конструктор бизнес-дашбордов | `products/simple_dashboard/product.yaml` | `https://simpledashboard.wpmix.net` |
...
├── product.yaml              # Метаданные, бот-токены, домены
├── CLAUDE.md                 # Полные инструкции (для разработчиков)
├── CLAUDE.md.template        # Клиентский шаблон (копируется в workspace юзера)
├── .claude/skills/           # Product-specific skills
├── showcases/                # Демо-примеры для галереи
└── icons/                    # Иконки и брендинг
```

**Клиентские CLAUDE.md шаблоны:**
- `products/{product_type}/CLAUDE.md.template` — product-specific (копируется в workspace юзера при онбординге)
- `botplatform/CLAUDE.md.example` — generic fallback
...
| **Claude Skills Marketplace** | `products/simple_dashboard/SKILL.md` | Standalone skill для Claude Code |
...
# Или все процессы из конфига
pm2 reload botplatform/ecosystem.config.js --update-env
```
