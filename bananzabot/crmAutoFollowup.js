...
// CHANGE: Generate short feature-highlight follow-up from @bananza_bot service
// WHY: Previous texts were too generic ("you stopped, need help?") — new style shows concrete feature relevant to user's niche
// REF: user request 2026-03-06 "от сервиса, тон дружеский и короткий, типа еще у нас есть такое"
async function generateFollowupMessage(userId, conversationSummary, stage, leadUsername) {
    const systemPrompt = 'Ты помощник для написания follow-up сообщений от сервиса @bananza_bot. Возвращай только текст сообщения, без пояснений.';

    // Реальные фичи bananzabot — выбирать по нише пользователя:
    // /broadcast → рассылки, акции, анонсы (магазины, клубы, курсы)
    // Уведомления → приём заявок, заказов, записей (сервисы, услуги)
    // Inline кнопки → выбор товара/тарифа/услуги (e-commerce, подписки)
    // Reply на уведомление → быстрый ответ клиенту без поиска чата (консультации, поддержка)
    // /mystats + /users → аналитика аудитории (после запуска)
    const featuresContext = `Реальные фичи @bananza_bot — выбери ОДНУ по нише:
- /broadcast — разослать сообщение всем пользователям бота разом: акция, анонс, напоминание. Хорошо для: магазинов, клубов, курсов, подписок.
- Уведомления о заявках — бот шлёт тебе уведомление когда клиент оставил заявку. Через /editbot. Хорошо для: услуги, запись, заказы.
- Inline кнопки — клиент выбирает товар/тариф одним тапом, без лишних слов. Через /editbot. Хорошо для: каталоги, тарифы, меню.
- Reply на уведомление — нажать Reply и ответ придёт клиенту от бота, не нужно искать чат. Хорошо для: консультации, поддержка, переписка.
- /mystats + /users — посмотреть аудиторию: сколько людей, список. Хорошо для: тех кто уже запустил бота.`;

    const userPrompt = `Напиши короткий follow-up от сервиса @bananza_bot.

Структура (строго, без пустых строк между предложениями):
1. Упомяни ОДНУ конкретную фичу — самую полезную для этой ниши (выбирай по подсказкам "Хорошо для:")
2. Одно предложение как она помогает их конкретному бизнесу
3. Мягкий призыв: "Ваш бот [тема] почти готов — запустим?" ИЛИ "Поможем настроить?"

Правила:
- Тон: дружелюбный и прямой — как сообщение от сервиса другу
- Длина: 200-300 символов
- НЕ спрашивай "что помешало" — не нужно
- НЕ предлагай "бесплатную помощь" — само собой
- НЕ используй markdown
- Пиши одним абзацем БЕЗ пустых строк внутри
- Имя пользователя: ${leadUsername ? leadUsername : 'неизвестно'} — если известно, начни с него, иначе с "Привет!"

${featuresContext}

Ниша/бизнес из диалога:
${conversationSummary}

Только текст сообщения (один абзац):`;
