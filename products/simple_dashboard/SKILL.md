...
// ── Data methods (основные) ──
SD.data.list(collection)                      // → GET all records
SD.data.create(collection, item)              // → POST, returns { id, ...item }
SD.data.update(collection, id, item)          // → PUT, returns updated record
SD.data.patch(collection, id, item)           // → PUT (alias for update)
SD.data.delete(collection, id)                // → DELETE, returns { deleted: id }
SD.data.getOne(collection, id)                // → GET single record or null
SD.data.upsert(collection, keyField, item)    // → create or update by keyField

// ── Data methods (deprecated — используй основные выше) ──
SD.data.get(collection)         // deprecated → используй SD.data.list()
SD.data.post(collection, item)  // deprecated → используй SD.data.create()
SD.data.put(collection, id, item) // deprecated → используй SD.data.update()
SD.data.del(collection, id)     // deprecated → используй SD.data.delete()
...
SD.admin.getMembers()           // → SD.data.list('members') + isOwner флаг
SD.admin.removeMember(email)    // → удалить из members + revokeAccess (только owner)
```
...
**`SD.data.list(collection)`** — массив всех записей:
```javascript
// SD.data.list('members') →
[
...
**`SD.data.create(collection, item)`** — создать запись (id генерируется автоматически):
```javascript
// SD.data.create('members', { name: "Ivan", email: "ivan@example.com" }) →
{ id: "m6a2f4", name: "Ivan", email: "ivan@example.com" }
```

**`SD.data.update(collection, id, item)`** — обновить запись:
```javascript
// SD.data.update('members', 'm6a2f4', { score: 42 }) →
{ id: "m6a2f4", name: "Ivan", email: "ivan@example.com", score: 42 }
```

**`SD.data.delete(collection, id)`** — удалить запись:
```javascript
// SD.data.delete('members', 'm6a2f4') →
{ deleted: "m6a2f4" }
```

**`SD.data.getOne(collection, id)`** — получить одну запись (не массив):
```javascript
// SD.data.getOne('members', 'm6a2f4') →
{ id: "m6a2f4", name: "Ivan", email: "ivan@example.com", joinedAt: "2026-03-06T12:00:00Z" }

// SD.data.getOne('members', 'nonexistent') →
null
```

**`SD.data.upsert(collection, keyField, item)`** — создать или обновить по уникальному полю:
```javascript
// Первый вызов (email не существует) — создаёт запись:
await SD.data.upsert('members', 'email', { email: 'ivan@example.com', name: 'Ivan' });
// → { id: "m6a2f4", email: "ivan@example.com", name: "Ivan" }

// Повторный вызов (email уже есть) — обновляет существующую запись:
await SD.data.upsert('members', 'email', { email: 'ivan@example.com', name: 'Ivan P.', lastSeen: '2026-03-06' });
// → { id: "m6a2f4", email: "ivan@example.com", name: "Ivan P.", lastSeen: "2026-03-06" }
```

**`SD.admin.getMembers()`** — список участников с isOwner флагом:
```javascript
// SD.admin.getMembers() →
[
  { id: "m6a2f4", email: "owner@example.com", name: "Owner", isOwner: true, joinedAt: "2026-03-01T10:00:00Z" },
  { id: "n7b3g5", email: "guest@example.com", name: "Guest", isOwner: false, joinedAt: "2026-03-06T12:00:00Z" }
]
```

**`SD.admin.removeMember(email)`** — удалить участника и отозвать доступ (только owner):
```javascript
await SD.admin.removeMember('guest@example.com');
// → удаляет запись из members collection + вызывает revokeAccess
...
document.addEventListener('sd:auth', async function() {
  ...
  // Зарегистрировать себя в списке (upsert предотвращает дубликаты при повторном логине)
  try {
    await SD.data.upsert('members', 'email', {
      name: user.name || '',
      email: user.email,
...
async function loadMembers() {
  try {
    var members = await SD.admin.getMembers();
    renderMembers(members);  // массив [{ id, name, email, isOwner, lastSeen }]
  } catch(e) {
    showEmpty();
  }
}
...
Если нужно показать owner'у кто имеет доступ и дать возможность удалить:
...
document.addEventListener('sd:auth', async function() {
  ...
  try {
    var members = await SD.admin.getMembers();
    // members = [{ id, email, name, isOwner, lastSeen }]
    members.forEach(function(m) {
      if (!m.isOwner) {
        renderUserRow(m.email, m.name, m.lastSeen);
      }
    });
  } catch(e) { console.warn('getMembers failed', e); }
});

function removeMember(email) {
  SD.admin.removeMember(email).then(function() {
...
async function submitForm(data) {
  try {
    var saved = await SD.data.create('submissions', {
      ...data,
...
async function updateRecord(id, changes) {
  await SD.data.update('submissions', id, changes);
}

async function deleteRecord(id) {
  await SD.data.delete('submissions', id);
  loadSubmissions();
}
```
