#!/usr/bin/env python3
"""
Smoke-test for @coderboxbot on protected production edge.
...
What we validate:
1) /start responds in English (no Cyrillic)
2) Sending a message persists history to /root/aisellusers/user_{id}/.history.json on the edge
3) Workspace folder exists and CLAUDE.md is created on the edge
...
BOT_USERNAME = "coderboxbot"
EXPECTED_LANGUAGE = "en"
TIMEOUT_SECONDS = 12

# Default: protected prod edge. Override if needed:
#   REMOTE_SSH_HOST=root@1.2.3.4 python3 tests/test_onboarding_coderboxbot.py
...
class Tester:
    ...
    async def send_and_wait(self, text: str) -> Message:
    ...
    async def test_start(self) -> bool:
        print("\n📋 Test 1: /start (English, no Cyrillic)")

        # Cleanup remote workspace to ensure we're checking fresh persistence.
        proc = self._ssh(f"rm -rf {self.user_dir} || true", timeout=10)
        self._check(proc.returncode == 0, "Remote cleanup via SSH", REMOTE_SSH_HOST)

        msg = await self.send_and_wait("/start")
        if not msg:
            return self._check(False, "Receive /start response")

        txt = msg.text or ""
        self._check(bool(txt.strip()), "Start response has text")
        self._check(not has_cyrillic(txt), "Start response has no Cyrillic", txt[:80].replace("\n", " "))
        self._check(
            any(k in txt.lower() for k in ["hello", "welcome", "help", "ready", "codex", "claude"]),
            "Start response looks English",
            txt[:80].replace("\n", " "),
        )
        return True

    async def test_persistence_and_workspace(self) -> bool:
        ...
        # Wait for remote .history.json to contain our marker.
        ...
        try:
            ...
            self._check(False, "History persisted on edge", str(e))

        # CHANGE: Check onboarding setting before expecting workspace/CLAUDE.md
        # WHY: Free tier bots don't create workspace
        ...
        if onboarding_enabled:
            ...
            self._check(proc.returncode == 0, "Workspace directory exists on edge (paid)", self.user_dir)

            claude_md = f"{self.user_dir}/CLAUDE.md"
            proc = self._ssh(f"test -f {claude_md}")
            self._check(proc.returncode == 0, "CLAUDE.md exists on edge (paid)", claude_md)

            proc = self._ssh(f"test -s {claude_md}")
            self._check(proc.returncode == 0, "CLAUDE.md is non-empty (paid)", claude_md)
        else:
            # Free tier: no workspace expected, but bot works
            self._check(True, "Free tier mode detected (no workspace expected)")
            # History may still be saved in a different location for free tier
            proc = self._ssh(f"test -d {self.user_dir} && test -f {self.user_dir}/.history.json")
            if proc.returncode == 0:
                self._check(True, "History persisted in workspace (free tier)")
            else:
                self._check(True, "Free tier bot works without workspace")

        return True

    async def test_pm2(self) -> bool:
        print("\n📋 Test 3: PM2 process online on edge")
        proc = self._ssh("pm2 ls | cat", timeout=10)
        out = (proc.stdout or "") + (proc.stderr or "")
        online = ("coderboxbot" in out) and ("online" in out)
        return self._check(online, "PM2 shows coderboxbot online", REMOTE_SSH_HOST)
